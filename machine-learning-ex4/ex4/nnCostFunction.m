...
for i=1:m;
  for k=1:num_labels;
    J = J + y1(i, k) * log(a2(i, k)) + (1-y1(i, k)) * log(1-a2(i, k));
  endfor
endfor
J = J * (-1/m);
...
Theta2_grad(:, 2:end) = (1/m) .* D_2(:, 2:end) + (lambda/m) .* Theta2(:, 2:end);

Theta1_grad(:, 1) = (1/m) .* D_1(:, 1);
Theta1_grad(:, 2:end) = (1/m) .* D_1(:, 2:end) + (lambda/m) .* Theta1(:, 2:end);
